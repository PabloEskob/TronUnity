...
namespace Opsive.BehaviorDesigner.AddOns.MovementPack.Runtime.Tasks
{
    ...
    [Opsive.Shared.Utility.Description(
        "Rotates towards the target rotation using Quaternion.RotateTowards. The rotation can either be specified by a Transform or rotation. " +
    ...
    {
        [Tooltip("Is the agent rotating in 2D?")] [SerializeField]
        protected bool m_Use2DPhysics;

        [Tooltip("The maximum number of angles the agent can rotate in a single tick (in degrees).")] [SerializeField]
        protected SharedVariable<float> m_MaxRotationDelta = 1;

        [Tooltip("Specifies the angle when the agent has arrived at the target rotation (in degrees).")] [SerializeField]
        protected SharedVariable<float> m_ArrivedAngle = 0.5f;

        [Tooltip("Should the rotation only affect the Y axis?")] [SerializeField]
        protected SharedVariable<bool> m_OnlyY = true;

        [Tooltip("The GameObject that the agent is rotating towards.")] [SerializeField]
        protected SharedVariable<GameObject> m_Target;
